...
{   
    ...
    if( pConfig->bSysClockSel ) {/* Select PLL as system clock */
        ...
        Clock_OSC_Enable();      /* Select OSC as system clock */
            
        /* 1.Configure REFDV register. The REFDV register provides a finer 
        ...
        /* 2.Configure POSTDIV register.The POSTDIV register controls the
             frequency ratio between the VCOCLK and PLLCLK. If POSTDIV = 0, 
             fVCO = fPLL */	
        POSTDIV = 0x00;
    
        /* 3.Configure SYNDR register.The SYNR register controls the 
             multiplication factor of the IPLL and selects the VCO
             frequency range. */
        fPLL = 2 * fBUS;
        if( !POSTDIV ) {
            fVCO = fPLL;  
    ...
    else {                       /* Select OSC as system clock */
        Clock_OSC_Enable();
    }
}